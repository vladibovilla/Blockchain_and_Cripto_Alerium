...
def add_transaction():
    ...
    if not all (key in json for key in transaction_keys):
        return 'Algun elemento en la trasaccion esta faltando', 400
    
    index = blockchain.add_transaction(json['sender'], json['receiver'], json['amount'])
    response = {'message':f'la transaccion sera añadida al Bloque{index}'}
    return jsonify(response), 201

#Paso 3: Desentralizando el Blockchain
...
def connect_node():
    ...
    response = {'message':'Todos los nodos estan conectados. Alerium Blockchain contiene los siguientes nodos:', 
